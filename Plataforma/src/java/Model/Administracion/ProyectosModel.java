    ...
    public Vector<Map> saveStep1(Map<String, String> values1, JSONArray espe, String update, String id_proyect){
        ...
        try {
            ...
            if(update.equals("1")){
                ...
                //Editar si el nombre es diferente al actual
                sql = "SELECT name \n" +
                        "  FROM ["+bd+"].[dbo].[ProyectsTemp]\n" +
                        "	where id = " + id_proyect;
                data = this.consultar(sql, conn);
                
                if(!(stringToBD(data.get(0).get("name").toString()).equals(values1.get("name")))){ // Si no es igual se edita
...
"  where pro.id_user  = " + id_user;*/
        ...
        String sql = "SELECT pro.id as id, pro.name as nombre, ["+bd+"].[dbo].getFlag(flag) as flag, lider, usu.name\n" +
"  FROM ["+bd+"].[dbo].[ProyectsTemp] pro\n" +
...
"  where pro.id_user  =" + id_user;
    ...
    public JSONObject  AprobarActividad(String id, String state) throws Exception{
       ...
       /* data = */this.ejecutarUpdate(sql,null);
       json.put("registro", 0);
