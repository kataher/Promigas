...
//P2 = get_Pres(parseFloat(P2),0, uni.bp_sel_bdp, "psia");
    ...
    while (sw > 0) {
        ...
        P2 = Math.pow(((-Math.pow((Q / (737.0 * Ef * Math.pow((Tb / Pb), 1.02) * Math.pow(D, 2.53))), 1.0 / 0.51) * (Z * Math.pow(G, 0.961) * Tf * Le) + Math.pow(P1a, 2)) / Math.pow(e, s)), 0.5);
...
//P1 = get_Pres(parseFloat(P1),0, uni.up_sel_bdp, "psia");
    ...
    var P1 = 0;
    while (sw > 0) {
        P1 = Math.pow(((Math.pow((Q / (737 * Ef * Math.pow((Tb / Pb), 1.02) * Math.pow(D, 2.53))), 1 / 0.51) * (Z * Math.pow(G, 0.961) * Tf * Le) + Math.pow(e, s) * Math.pow(P2a, 2))), 0.5);
        Pavg = (2 / 3) * (P1 + P2 - P1 * P2 / (P1 + P2)); 
        Z = 1 / (1 + (Pavg * 344400 * Math.pow(10, (1.785 * G)) / Math.pow(Tf, 3.825)));
        s = 0.0375 * G * (h2 / Z - h1 / Z) / Tf;
        if (s === 0) {
...
function upstream_cl_form(vari, uni) {
    ...
    while (sw > 0) {
        ...
        console.log((Z * G * Tf * Le) + Math.pow(e, s));
        console.log(Math.pow(e, s));
        console.log((Z * G * Tf * Le));
        console.log(Z);
        console.log(G);
        console.log(Tf);
        console.log(Le);
        console.log(Math.pow((Q / (38.77 * F * (Tb / Pb) * Math.pow(D, 2.5))), 2) * (Z * G * Tf * Le) + Math.pow(e, s) * Math.pow(P2a, 2));
...
function internal_wdp_form(vari, uni) {
    ...
    var D = Math.pow((Q / (433.5 * Ef * (Tb / Pb) * Math.pow(((Math.pow(P1a, 2) - Math.pow(e, s) * Math.pow(P2a, 2)) / (Z * G * Tf * Le)), 0.5))), 1 / 2.667);
    D = puntos(D, 3);
    /* salida
     D  = Internal Pipe Diameter
     */
...
function SteelpipelinesCH(vari, uni) {
    ...
    Es = get_Presf(Es, uni.youngModulusForSteel1_sel_spch, 'psi');
    P = get_Presf(P, uni.operatingPressure_sel_spch, 'psi');
    p = get_Presf(p, uni.operatingPressure_sel_spch, 'psi');
    T1 = get_Temp(T1, uni.operatingTemperature_sel_spch, 'F');
    T2 = get_Temp(T2, uni.installationTemperature_sel_spch, 'F');
    
    D = get_Long(D, uni.pipeOutsideDiameter_sel_spch, 'in');
    tw = get_Long(tw, uni.pipeWallThickness_sel_spch, 'in');
...
function SteelpipelinesCR(vari, uni) {
    ...
    Es = get_Presf(Es, uni.youngModulusForSteel1_sel_spcr, 'psi');
    p = get_Presf(p, uni.operatingPressure_sel_spcr, 'psi');
    T1 = get_Temp(T1, uni.operatingTemperature_sel_spcr, 'F');
    T2 = get_Temp(T2, uni.installationTemperature_sel_spcr, 'F');
    
    D = get_Long(D, uni.pipeOutsideDiameter_sel_spcr, 'in');
    tw = get_Long(tw, uni.pipeWallThickness_sel_spcr, 'in');
    H = get_Long(H, uni.pipeDepth_sel_spcr, 'ft');
    Bd = get_Long(Bd, uni.boredDiameter_sel_spcr, 'in');
    
    Lg = get_Long(Lg, uni.distanceOfGirthWeldFromTrackCenterline_sel_spcr, 'psi');
...
function interpolate(valueX, lowerXBound, upperXBound, lowerYBound, upperYBound) {
    var Xdistance = upperXBound - lowerXBound;
    var percent = (valueX - lowerXBound) / Xdistance;
    
    var distance = upperYBound - lowerYBound;
    return lowerYBound + (percent * distance);
}

function interpolateArrays(valueX, lowerXBound, upperXBound, lowerY, upperY) {
    var array = [];
    
    for (var i = 0; i < lowerY.length; ++i) {
        array.push(interpolate(valueX, lowerXBound, upperXBound, lowerY[i], upperY[i]));
    }
...
function  valorX0(xo, h, val) {
    ...
    if (val == 'Khe') {
        ...
        if (h > 0.2 && h < 0.5) {
            var lowerY = [11754.2, 9631.99, 7734.05, 6182.34, 4975.81, 4052.84, 3345.77, 2798.92, 2370.56, 2030.46, 1756.82, 1533.89, 1350.15, 1197.11, 1068.39, 959.169, 865.739, 785.227, 715.376, 654.396, 600.856, 553.6, 511.687, 474.343, 440.931, 410.919, 383.863, 359.387, 337.176, 316.957, 298.501, 281.609, 266.11, 251.854, 238.712, 226.571, 215.333, 204.91, 195.224, 186.21];
            var upperY = [7702.170, 6682.500, 5810.950, 4984.920, 4236.260, 3590.650, 3050.760, 2605.940, 2241.130, 1941.540, 1694.360, 1489.090, 1317.420, 1172.780, 1050.030, 945.107, 854.833, 776.667, 708.584, 648.954, 596.456, 550.012, 508.737, 471.901, 438.895, 409.212, 382.422, 358.165, 336.133, 316.064, 297.732, 280.944, 265.532, 251.350, 238.271, 226.185, 214.992, 204.609, 194.958, 185.973];
            y = interpolateArrays(h, 0.2, 0.5, lowerY, upperY);
        }
        if (h == 0.5) {
        ...
        if (h > 0.5 && h < 1) {
            var lowerY = [7702.170, 6682.500, 5810.950, 4984.920, 4236.260, 3590.650, 3050.760, 2605.940, 2241.130, 1941.540, 1694.360, 1489.090, 1317.420, 1172.780, 1050.030, 945.107, 854.833, 776.667, 708.584, 648.954, 596.456, 550.012, 508.737, 471.901, 438.895, 409.212, 382.422, 358.165, 336.133, 316.064, 297.732, 280.944, 265.532, 251.350, 238.271, 226.185, 214.992, 204.609, 194.958, 185.973];
            var upperY = [5695.01, 4836.42, 4316.75, 3872.92, 3447.67, 3043.84, 2673.71, 2344.72, 2058.26, 1811.77, 1600.9, 1420.78, 1266.75, 1134.67, 1020.98, 922.696, 837.34, 762.868, 697.589, 640.112, 589.284, 544.148, 503.907, 467.894, 435.55, 406.402, 380.048, 356.149, 334.412, 314.588, 296.46, 279.844, 264.576, 250.517, 237.542, 225.544, 214.428, 204.11, 194.516, 185.58];
            y = interpolateArrays(h, 0.5, 1, lowerY, upperY);
        }
        if (h == 1.0) {
            y = [5695.01, 4836.42, 4316.75, 3872.92, 3447.67, 3043.84, 2673.71, 2344.72, 2058.26, 1811.77, 1600.9, 1420.78, 1266.75, 1134.67, 1020.98, 922.696, 837.34, 762.868, 697.589, 640.112, 589.284, 544.148, 503.907, 467.894, 435.55, 406.402, 380.048, 356.149, 334.412, 314.588, 296.46, 279.844, 264.576, 250.517, 237.542, 225.544, 214.428, 204.11, 194.516, 185.58];
        }
        if (h > 1 && h < 2) {
            var lowerY = [5695.01, 4836.42, 4316.75, 3872.92, 3447.67, 3043.84, 2673.71, 2344.72, 2058.26, 1811.77, 1600.9, 1420.78, 1266.75, 1134.67, 1020.98, 922.696, 837.34, 762.868, 697.589, 640.112, 589.284, 544.148, 503.907, 467.894, 435.55, 406.402, 380.048, 356.149, 334.412, 314.588, 296.46, 279.844, 264.576, 250.517, 237.542, 225.544, 214.428, 204.11, 194.516, 185.58];
            var upperY = [4496.67, 3572.13, 3122.36, 2837.51, 2604.99, 2386.84, 2174.9, 1971.32, 1780.21, 1604.58, 1445.79, 1303.83, 1177.83, 1066.43, 968.116, 881.359, 804.719, 736.896, 676.737, 623.235, 575.52, 532.841, 494.556, 460.111, 429.033, 400.913, 375.401, 352.193, 331.03, 311.682, 293.954, 277.672, 262.688, 248.868, 236.098, 224.275, 213.309, 203.121, 193.638, 184.8];
            y = interpolateArrays(h, 1, 2, lowerY, upperY);
        }
        if (h == 2.0) {
    ...
    if (val == 'KLr') {
        ...
        if (h > 5 && h < 10) {
            var lowerY = [550.429, 496.267, 456.622, 426.767, 403.063, 382.836, 364.244, 346.157, 328.028, 309.766, 291.611, 274.01, 257.484, 242.511, 229.391];
            var upperY = [373.514, 341.162, 314.788, 293.178, 275.265, 260.13, 246.998, 235.241, 224.377, 214.07, 204.132, 194.519, 185.336, 176.83, 169.399];
            y = interpolateArrays(h, 5, 10, lowerY, upperY);
        }
        if (h == 10) {
            y = [373.514, 341.162, 314.788, 293.178, 275.265, 260.13, 246.998, 235.241, 224.377, 214.07, 204.132, 194.519, 185.336, 176.83, 169.399];
        }
        if (h > 10 && h < 20) {
            var lowerY = [373.514, 341.162, 314.788, 293.178, 275.265, 260.13, 246.998, 235.241, 224.377, 214.07, 204.132, 194.519, 185.336, 176.83, 169.399];
            var upperY = [255.664, 231.372, 213.004, 199.108, 188.441, 179.964, 172.845, 166.457, 160.378, 154.393, 148.493, 142.872, 137.932, 134.282, 132.732];
            y = interpolateArrays(h, 10, 20, lowerY, upperY);
        }
        if (h == 20) {
    ...
    if (val == 'GLr') {
        ...
        if (h > 4 && h < 6) {
            var lowerY = [2.60022, 2.38986, 2.20672, 2.04778, 1.91026, 1.79166, 1.68971, 1.60237, 1.52779, 1.46431, 1.41046, 1.36492, 1.32652, 1.29421, 1.26709, 1.24436, 1.22531, 1.20933, 1.19591, 1.18459, 1.17498, 1.16676, 1.15965, 1.15343, 1.14791, 1.14292, 1.13835, 1.1341, 1.13008, 1.12622, 1.12249, 1.11885, 1.11526, 1.1117, 1.10815, 1.1046, 1.10104, 1.09745, 1.09382, 1.09014, 1.08639, 1.08255, 1.07861, 1.07455, 1.07036, 1.066, 1.06147, 1.05674, 1.05179, 1.04661, 1.04118, 1.03549, 1.02952, 1.02328, 1.01676, 1.00996, 1.0029, 0.99557, 0.988003, 0.980215, 0.972231, 0.96408, 0.955794, 0.947405, 0.938946, 0.930452, 0.92195, 0.913465, 0.905014, 0.896603, 0.888224, 0.879852, 0.87144, 0.862917, 0.85418, 0.845088, 0.835461, 0.82507, 0.81363, 0.800793, 0.786141];
            var upperY = [2.26089, 1.96607, 1.74260, 1.57503, 1.45068, 1.35923, 1.29238, 1.24351, 1.20747, 1.18025, 1.15885, 1.14105, 1.12528, 1.11046, 1.09591, 1.08124, 1.06628, 1.051, 1.03547, 1.01983, 1.00421, 0.988781, 0.973672, 0.958994, 0.944824, 0.931203, 0.91814, 0.905609, 0.893563, 0.88193, 0.870625, 0.859555, 0.848625, 0.837746, 0.826834, 0.815822, 0.804657, 0.793305, 0.781751, 0.769999, 0.758074, 0.746014, 0.733875, 0.721721, 0.709625, 0.697662, .685906, 0.674428, 0.663289, 0.652536, 0.642204, 0.632308, 0.622845, .613794, 0.605113, 0.596745, 0.588618, 0.580649, 0.572749, 0.56483, 0.556809, 0.548615, 0.540197, 0.531531, 0.522623, 0.513515, 0.50429, 0.495068, 0.486007, 0.477295, 0.469135, 0.461729, 0.455253, 0.44982, 0.44544, 0.441956, 0.438983, 0.43582, 0.431348, 0.42391, 0.41117];
            y = interpolateArrays(h, 4, 6, lowerY, upperY);
        }
        if (h == 6) {
    ...
    if (val == 'GHr') {
        ...
        if (h > 6 && h < 10) {
            var lowerY = [1.00232, 1.07802, 1.07173, 1.0258, 0.966078, 0.906428, 0.852587, 0.805381, 0.763284, 0.724323, 0.687335, 0.652564, 0.62161, 0.596724, 0.579453];
            var upperY = [0.682392, 0.724973, 0.721385, 0.695564, 0.662114, 0.628902, 0.599249, 0.573731, 0.551584, 0.531721, 0.513342, 0.496163, 0.480242, 0.465415, 0.450335];
            y = interpolateArrays(h, 6, 10, lowerY, upperY);
        }
        if (h == 10) {
        ...
        if (h > 10 && h < 14) {
            var lowerY = [0.682392, 0.724973, 0.721385, 0.695564, 0.662114, 0.628902, 0.599249, 0.573731, 0.551584, 0.531721, 0.513342, 0.496163, 0.480242, 0.465415, 0.450335];
            var upperY = [0.476148, 0.501807, 0.500855, 0.486615, 0.467585, 0.448597, 0.431835, 0.417725, 0.405677, 0.394696, 0.383854, 0.372623, 0.361076, 0.34994, 0.340525];
            y = interpolateArrays(h, 10, 14, lowerY, upperY);
        }
        if (h == 14) {
    ...
    if (val == 'KHr') {
        ...
        if (h > 5 && h < 10) {
            var lowerY = [449.362, 482.496, 450.183, 385.788, 314.03, 249.998, 200.052, 164.009, 137.997, 117.366, 99.039, 82.702, 70.2177, 62.6448, 54.2606];
            var upperY = [313.642, 340.266, 330.47, 300.101, 260.713, 220.167, 183.237, 152.207, 127.477, 108.163, 92.7008, 79.4441, 67.2705, 56.1817, 47.9058];
            y = interpolateArrays(h, 5, 10, lowerY, upperY);
        }
        if (h == 10) {
        ...
        if (h > 10 && h < 20) {
            var lowerY = [313.642, 340.266, 330.47, 300.101, 260.713, 220.167, 183.237, 152.207, 127.477, 108.163, 92.7008, 79.4441, 67.2705, 56.1817, 47.9058];
            var upperY = [196.14, 230.733, 238.024, 227.915, 207.969, 183.697, 158.857, 135.738, 115.46, 98.2634, 83.798, 71.4199, 60.4816, 50.6243, 42.0704];
            y = interpolateArrays(h, 10, 20, lowerY, upperY);
        }
        if (h == 20) {
    ...
    if (val == 'Ghh') {
        ...
        if (h > 4 && h < 6) {
            var lowerY = [1.42094, 1.49553, 1.43717, 1.30637, 1.20534, 1.15815, 1.1372, 1.11494, 1.08766, 1.06612, 1.05378, 1.03972, 1.01779, 1.00791, 0.998545];
            var upperY = [1.08243, 1.1375, 1.11651, 1.04162, 0.973383, 0.931983, 0.906182, 0.880995, 0.852692, 0.825879, 0.802659, 0.777945, 0.748065, 0.721705, 0.693126];
            y = interpolateArrays(h, 4, 6, lowerY, upperY);
        }
        if (h == 6) {
        ...
        if (h > 6 && h < 8) {
            var lowerY = [1.08243, 1.1375, 1.11651, 1.04162, 0.973383, 0.931983, 0.906182, 0.880995, 0.852692, 0.825879, 0.802659, 0.777945, 0.748065, 0.721705, 0.693126];
            var upperY = [0.843088, 0.888621, 0.88312, 0.843053, 0.797324, 0.762494, 0.739748, 0.721955, 0.702358, 0.679635, 0.65728, 0.638429, 0.620475, 0.596993, 0.577715];
            y = interpolateArrays(h, 6, 8, lowerY, upperY);
        }
        if (h == 8) {
        ...
        if (h > 8 && h < 10) {
            var lowerY = [0.843088, 0.888621, 0.88312, 0.843053, 0.797324, 0.762494, 0.739748, 0.721955, 0.702358, 0.679635, 0.65728, 0.638429, 0.620475, 0.596993, 0.577715];
            var upperY = [0.66855, 0.717437, 0.710884, 0.681457, 0.648451, 0.620782, 0.600141, 0.584134, 0.569091, 0.552275, 0.533161, 0.513523, 0.496014, 0.48094, 0.460952];
            y = interpolateArrays(h, 8, 10, lowerY, upperY);
        }
        if (h == 10) {
    ...
    if (val == 'Glh') {
        ...
        if (h > 3 && h < 6) {
            var lowerY = [2.60022, 2.38986, 2.20672, 2.04778, 1.91026, 1.79166, 1.68971, 1.60237, 1.52779, 1.46431, 1.41046, 1.36492, 1.32652, 1.29421, 1.26709, 1.24436, 1.22531, 1.20933, 1.19591, 1.18459, 1.17498, 1.16676, 1.15965, 1.15343, 1.14791, 1.14292, 1.13835, 1.1341, 1.13008, 1.12622, 1.12249, 1.11885, 1.11526, 1.1117, 1.10815, 1.1046, 1.10104, 1.09745, 1.09382, 1.09014, 1.08639, 1.08255, 1.07861, 1.07455, 1.07036, 1.066, 1.06147, 1.05674, 1.05179, 1.04661, 1.04118, 1.03549, 1.02952, 1.02328, 1.01676, 1.00996, 1.0029, 0.99557, 0.988003, 0.980215, 0.972231, 0.96408, 0.955794, 0.947405, 0.938946, 0.930452, 0.92195, 0.913465, 0.905014, 0.896603, 0.888224, 0.879852, 0.87144, 0.862917, 0.85418, 0.845088, 0.835461, 0.82507, 0.81363, 0.800793, 0.786141];
            var upperY = [2.58652, 2.35029, 2.14695, 1.97241, 1.82299, 1.69541, 1.58672, 1.49431, 1.41587, 1.34934, 1.29294, 1.24511, 1.20447, 1.16985, 1.14025, 1.1148, 1.09278, 1.07358, 1.05669, 1.04169, 1.02826, 1.01611, 1.00503, 0.994867, 0.985488, 0.9768, 0.968737, 0.961249, 0.954304, 0.947875, 0.941945, 0.936499, 0.931519, 0.926988, 0.922883, 0.919177, 0.915838, 0.912826, 0.910095, 0.907596, 0.905271, 0.903059, 0.900898, 0.89872, 0.896459, 0.894049, 0.891428, 0.888534, 0.885313, 0.881719, 0.877711, 0.873261, 0.86835, 0.862969, 0.857125, 0.850836, 0.84413, 0.837053, 0.829658, 0.822012, 0.814191, 0.806277, 0.798358, 0.790524, 0.782863, 0.775457, 0.768376, 0.761673, 0.75538, 0.749495, 0.74398, 0.738748, 0.733654, 0.728488, 0.722955, 0.716672, 0.709146, 0.699764, 0.687775, 0.672274, 0.652183];
            y = interpolateArrays(h, 3, 6, lowerY, upperY);
        }
        if (h == 6) {
        ...
        if (h > 6 && h < 8) {
            var lowerY = [2.58652, 2.35029, 2.14695, 1.97241, 1.82299, 1.69541, 1.58672, 1.49431, 1.41587, 1.34934, 1.29294, 1.24511, 1.20447, 1.16985, 1.14025, 1.1148, 1.09278, 1.07358, 1.05669, 1.04169, 1.02826, 1.01611, 1.00503, 0.994867, 0.985488, 0.9768, 0.968737, 0.961249, 0.954304, 0.947875, 0.941945, 0.936499, 0.931519, 0.926988, 0.922883, 0.919177, 0.915838, 0.912826, 0.910095, 0.907596, 0.905271, 0.903059, 0.900898, 0.89872, 0.896459, 0.894049, 0.891428, 0.888534, 0.885313, 0.881719, 0.877711, 0.873261, 0.86835, 0.862969, 0.857125, 0.850836, 0.84413, 0.837053, 0.829658, 0.822012, 0.814191, 0.806277, 0.798358, 0.790524, 0.782863, 0.775457, 0.768376, 0.761673, 0.75538, 0.749495, 0.74398, 0.738748, 0.733654, 0.728488, 0.722955, 0.716672, 0.709146, 0.699764, 0.687775, 0.672274, 0.652183];
            var upperY = [2.59898, 2.29373, 2.05243, 1.86143, 1.7096, 1.58798, 1.48945, 1.40841, 1.34052, 1.28249, 1.23186, 1.18685, 1.1462, 1.10908, 1.07493, 1.04342, 1.01438, 0.987712, 0.963397, 0.94142, 0.921763, 0.90439, 0.889235, 0.876196, 0.865139, 0.855893, 0.848263, 0.842031, 0.836965, 0.832827, 0.829382, 0.826402, 0.823677, 0.821018, 0.818263, 0.815279, 0.811965, 0.808255, 0.804113, 0.799535, 0.794545, 0.789192, 0.783544, 0.777686, 0.771711, 0.765715, 0.759794, 0.754035, 0.748516, 0.743293, 0.738406, 0.733869, 0.729673, 0.725783, 0.722141, 0.718665, 0.715257, 0.711804, 0.708186, 0.704283, 0.699981, 0.695183, 0.689815, 0.683834, 0.677237, 0.670067, 0.662411, 0.654407, 0.646236, 0.638116, 0.630287, 0.622986, 0.616422, 0.610728, 0.605916, 0.601804, 0.597938, 0.593494, 0.587153, 0.576966, 0.560186];
            y = interpolateArrays(h, 6, 8, lowerY, upperY);
        }
        if (h == 8) {
    ...
    if (val == 'Klh') {
        ...
        if (h > 5 && h < 10) {
            var lowerY = [14.4518, 14.981, 14.311, 13.1071, 11.7946, 10.6131, 9.66538, 8.96102, 8.45401, 8.07528, 7.75964, 7.46721, 7.19934, 7.00917, 7.00652];
            var upperY = [9.69573, 10.1009, 9.82502, 9.17903, 8.38693, 7.5984, 6.90122, 6.33374, 5.89735, 5.56893, 5.31335, 5.09592, 4.89482, 4.71364, 4.59377];
            y = interpolateArrays(h, 5, 10, lowerY, upperY);
        }
        if (h == 10) {
        ...
        if (h > 10 && h < 20) {
            var lowerY = [9.69573, 10.1009, 9.82502, 9.17903, 8.38693, 7.5984, 6.90122, 6.33374, 5.89735, 5.56893, 5.31335, 5.09592, 4.89482, 4.71364, 4.59377];
            var upperY = [5.99465, 6.51714, 6.49606, 6.17045, 5.71035, 5.22739, 4.78539, 4.411, 4.10428, 3.84931, 3.62484, 3.41482, 3.21909, 3.06395, 3.01275];
            y = interpolateArrays(h, 10, 20, lowerY, upperY);
        }
        if (h == 20) {
    ...
    if (val == 'KHh') {
        ...
        if (h > 5 && h < 10) {
            var lowerY = [18.9393, 20.6162, 19.1077, 16.2715, 13.2193, 10.5219, 8.3843, 6.79039, 5.61778, 4.72208, 3.9911, 3.36883, 2.84924, 2.43984, 2.09501];
            var upperY = [12.3704, 14.8235, 14.2143, 12.4365, 10.4982, 8.81739, 7.46468, 6.35108, 5.3628, 4.44192, 3.61338, 2.95824, 2.53304, 2.23547, 1.61628];
            y = interpolateArrays(h, 5, 10, lowerY, upperY);
        }
        if (h == 10) {
        ...
        if (h > 10 && h < 20) {
            var lowerY = [12.3704, 14.8235, 14.2143, 12.4365, 10.4982, 8.81739, 7.46468, 6.35108, 5.3628, 4.44192, 3.61338, 2.95824, 2.53304, 2.23547, 1.61628];
            var upperY = [6.82618, 9.21612, 9.69183, 9.22279, 8.37056, 7.41976, 6.48525, 5.59552, 4.75234, 3.96662, 3.27045, 2.7055, 2.28744, 1.94681, 1.4459];
            y = interpolateArrays(h, 10, 20, lowerY, upperY);
        }
        if (h == 20) {
