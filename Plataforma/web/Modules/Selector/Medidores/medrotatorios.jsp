...
<html>
    <head>  
        ...
        <script src="../../../bower_components/morrisjs/morris.js">F</script>
                                ...
                                <div class="panel-body">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            ...
                                            <div class="form-group">
                                                <label id="lbl_capa_mer">Maximum Capacity of Measurement (Optimum):</label>            
                                            ...
                                            <div class="form-group">
                                                <label id="lbl_mcapae_mer">Maximum Capacity of Measurement(Evaluated):</label>            
        ...
        <script>
            ...
            function load_datasheet_mer() {
                ...
                if ($("#po_sel_mer").val().split(",")[1] === "psig") {
                    pres = Number($("#presionop_mer").val().replace(",",""));
                } else {
                    pres = Number($("#presionop_mer").val().replace(",","")) * 14.504;
                }
            ...
            function setGrafica_mer() {
                ...
                var caprefop = parseFloat($("#capaopt_mer").val().replace(",",""));
                ...
                while (inicio <= tope) {
                   valX[j] = inicio;
                   valOpY[j] = (inicio+presb*aux)/(presb*aux)*caprefop; 
                   valEvY[j] = (inicio + presb * aux) / (presb * aux) * caprefev;
                   valCapa[j] = parseFloat($("#flujomax_mer").val().replace(",",""));
                   inicio += paso;
                   j++;
                }
               
                var max = Math.max(valOpY[valOpY.length - 1], valEvY[valEvY.length - 1]);
               
                var pOpe = {
                    x: [parseFloat($("#presionop_mer").val()), parseFloat($("#presionop_mer").val())],
                    y: [0, max],
                    mode: 'lines',
                ...
                data = [opt, eval, capa, pOpe];
                
                var layout = {
                    title: 'Capacity Rotatory Meters',
                    xaxis: {
                        title: 'Capacity [SCFH]'
                    },
                    yaxis: {
                ...
                Plotly.newPlot('graficaLineal_mer', data, layout);
            ...
            function onchange_serie_mer(swclear) {
               ...
               if (swclear) {
                ...
                var op = $("#se_sel_mer").val().split(",")[1];

                if (op === "imperial1" || op === "imperial2") {
                ...
                $.ajax({
                    ...
                    success: function (data, status, request) {
                        var newHtml = "<select class=\"form-control\" name=\"mo_sel_mer\" id=\"mo_sel_mer\" onchange=\"onchange_mod_mer()\">" + data;
                        $("#div_mo_sel_mer").html(newHtml);

                        var opc = $("#mo_sel_mer").val().split(",")[1];
                        $("#capa_mer").val(opc);
                    },
                    error: function (xhr, ajaxOptions, err) {
                        show_OkDialog($("#error_Dialog_mer"), "Error");
            ...
            function onchange_mod_mer() {
                cleanOut_mer();
                var op = $("#mo_sel_mer").val().split(",")[1];
                $("#capa_mer").val(op);
            }
