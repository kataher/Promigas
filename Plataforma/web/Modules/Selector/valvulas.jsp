 ...
 <input type="button" id="datasheetgen_val" name="datasheetgen_val" value="Generate DataSheet" onclick="load_datasheet_val()" class="btn btn-success btn-block">   
            ...
            <script>
                ...
                function setGrafica_val(){
                    ...
                    var temp = $("#op1g_val").val();
                    var flujo = $("#op2g_val").val();
                    
                    var data = [];
                    var ini = parseFloat($("#presiondis_val").val()) - 30;
                    var fin = parseFloat($("#presiondis_val").val()) + 40;
                    var areacom = parseFloat(parseFloat($("#arearecom_val").val()));
                    var kd = parseFloat(parseFloat($("#coefdes_val").val()));
                    var minp = parseFloat(parseFloat($("#nivelsobmin_val").val()));
                    var maxp = parseFloat(parseFloat($("#nivelsobmax_val").val()));
                    var c = parseFloat(parseFloat($("#cval_val").val()));
                    var kb = parseFloat(parseFloat($("#factcor_val").val()));
                    var z = parseFloat(parseFloat($("#factcomp_val").val()));
                    var pesomol = parseFloat(parseFloat($("#pesom_val").val()));
                    
                    var eve  = 0;
                    for(var i = ini; i < fin; i = i + 10){
                        var a = 6.32* areacom * kd *(i+ Math.max(minp,maxp) + 14.65 )*c*kb/(Math.pow(z*temp*pesomol,0.5));
                        data[data.length] = { x: i.toString(), a: a.toFixed(0), b : null};
                        
                        if(i.toString() === $("#presiondis_val").val()){
                            data[data.length - 1 ].b = flujo;
                            eve = data.length - 1;
                        }
                   ...
                   Morris.Line({
                            element: 'graficaLineal_val',
                            data: data,
                            xkey: 'x',
                            ykeys: ['a', 'b'],
                            labels: ['Capacidad de la vàlvula', 'Punto de Descarga'],
                            events: [
                              eve
                            ],
                            parseTime:false
